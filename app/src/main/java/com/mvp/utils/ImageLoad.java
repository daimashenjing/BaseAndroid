...
public class ImageLoad {
    ...
    private DisplayImageOptions getDefaultOptions() {
        return new DisplayImageOptions.Builder().showImageOnLoading(R.mipmap.icon_addpic_unfocused2) // 设置图片下载期间显示的图片
                .showImageForEmptyUri(R.mipmap.icon_addpic_unfocused2) // 设置图片Uri为空或是错误的时候显示的图片
                .showImageOnFail(R.mipmap.icon_addpic_unfocused2) // 设置图片加载或解码过程中发生错误显示的图片
    ...
    private DisplayImageOptions getPhotoOptions() {
        return new DisplayImageOptions.Builder().showImageOnLoading(R.mipmap.icon_addpic_unfocused2) // 设置图片下载期间显示的图片
                .showImageForEmptyUri(R.mipmap.icon_addpic_unfocused) // 设置图片Uri为空或是错误的时候显示的图片
                .showImageOnFail(R.mipmap.icon_addpic_unfocused2) // 设置图片加载或解码过程中发生错误显示的图片
                .bitmapConfig(Bitmap.Config.RGB_565)// 设置图片的解码类型
                .cacheInMemory(true) // 设置下载的图片是否缓存在内存中
                .cacheOnDisk(true) // 设置下载的图片是否缓存在SD卡中
